...
public class BigDecimalValueProvider extends AbstractClassifyDataProvider implements ClassifyDataProvider {
	...
	protected List<ClassifyDataParameter> getProviderParameterValues(Field dtoField, ClassifyDataParameter dataParameter,
		...
		if (CONTAINS_ONE_OF.equals(dataParameter.getOperator()) || EQUALS_ONE_OF.equals(dataParameter.getOperator())) {
			throw new ClientException(errorMessage("error.unsupported_type_filtration", BigDecimal.class));
		}
		dataParameter.setValue(filterParam.getBigDecimalValue());

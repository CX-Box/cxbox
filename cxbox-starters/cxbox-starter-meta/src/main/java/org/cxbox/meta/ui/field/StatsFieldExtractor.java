...
import org.cxbox.meta.ui.model.json.options.WidgetOptionsStats;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Component;

@Component
public class StatsFieldExtractor extends BaseFieldExtractor {

	public StatsFieldExtractor(@Autowired LinkFieldExtractor linkFieldExtractor) {
	...
	public Set<BcField> extract(final WidgetDTO widget) {
		...
		// add default fields from the option stat
		widgetFields.addAll(linkFieldExtractor.extractDefaultFieldsLinkToFields(
				widget,
				Optional.ofNullable(extractWidgetOptions(widget))
						.map(WidgetOptions::getStats)
						.orElse(null),
				WidgetOptionsStats.class));
